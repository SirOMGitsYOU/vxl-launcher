...
export async function skinPreviewKey(
  ...
  const { textureUrl, attachables, emote, ...rest } = request;
  return sha1(
    canonical({
      v: RENDER_VERSION,
      ...
      attachables: (attachables ?? []).map((attachable) => ({
        id: attachable.meta.id,
        geo: attachable.assets.geo,
        texture: attachable.assets.texture,
      })),
      ...rest,
    }),
  );
...
function getRenderer(width: number, height: number): SizedRenderer {
  ...
  if (!entry) {
    entry = {
      renderer: new SnapshotRenderer(undefined, width, height),
      disposeTimer: null,
      inFlight: 0,
    };
