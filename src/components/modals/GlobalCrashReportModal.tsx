...
import { Alert, Badge, Button, Card } from "../ui-v2";
...
export function GlobalCrashReportModal() {
  ...
  const [displayedCrashReportContent, setDisplayedCrashReportContent] = useState<
    string | undefined
  >(undefined);
  ...
  useEffect(() => {
    if (crashData?.profile_id) {
      ...
      hasFetchedCrashReportRef.current = false;
    } else {
      setProfileName("");
      setMclogsUrl(null);
      setNoriskReportSubmitted(false);
  ...
  useEffect(() => {
    const focusRelevantWindow = async () => {
      if (isCrashModalOpen && crashData?.process_id) {
        ...
        console.log(
          `Crash modal open for process ${crashedProcessId}. Attempting to focus relevant window.`,
        );
        ...
        try {
          ...
          console.warn(
            `Log window with label ${logWindowLabel} not found for process ${crashedProcessId}:`,
            e,
          );
        }
    ...
    try {
      ...
      if (!currentMclogsUrl) {
        ...
        if (displayedCrashReportContent && displayedCrashReportContent.trim() !== "") {
          ...
          toast.loading("Preparing combined log (crash report + latest.log)...", {
            id: mainToastId,
          });
        }
      ...
      if (currentMclogsUrl && !noriskReportSubmitted) {
        ...
        try {
          ...
          toast.success(`Report submitted. Log URL: ${currentMclogsUrl} (Copying failed)`, {
            id: mainToastId,
          });
        }
      ...
      toast.error(message, { id: mainToastId });
  ...
  const primaryButtonText =
    mclogsUrl && noriskReportSubmitted ? "Copy log URL" : "Upload logs & report";
  ...
  const modalFooter = (
    <div className="flex gap-2">
      <Button
        ...
        icon={
          <Icon
            icon={mclogsUrl && noriskReportSubmitted ? "solar:copy-bold" : "solar:upload-bold"}
            className="h-4 w-4"
          />
        }
      >
        {isProcessing ? "Processing..." : primaryButtonText}
      </Button>
      <Button
        ...
        icon={<Icon icon="solar:chat-round-dots-bold" className="h-4 w-4" />}
      >
        Contact support
      </Button>
    </div>
  ...
  const resolvedProfileName =
    crashData.process_metadata?.profile_name || profileName || "Loading...";

  return (
    <Modal
      title="Minecraft crash report"
      titleIcon={
        <Icon icon="solar:danger-triangle-bold" className="h-5 w-5 text-red-400" />
      }
      titleSubtitle={
        <p className="text-sm text-[var(--text-secondary)]">Profile: {resolvedProfileName}</p>
      }
      ...
      <div className="space-y-4 p-6 select-none">
        <Card className="p-5 text-center">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
            ...
            {isListeningForCrashContent && !displayedCrashReportContent && (
        ...
        <Alert tone="info">
          Upload your logs to help diagnose the issue, or contact support on Discord if the problem
          persists.
        </Alert>
      </div>
    </Modal>
