...
export function CustomDropdown({
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {label && (
        <label className="block text-white  text-3xl lowercase mb-2">
          {label}
        </label>
      )}
      ...
      <button
        ...
        className={`flex items-center gap-2 transition-all duration-200 focus:outline-none focus:ring-0 focus:border-transparent ${
          variant === 'search'
            ? 'w-full justify-between bg-black/50 rounded-lg px-4 py-3 border border-white/10 hover:border-white/20 text-white  text-xl'
            : label
            ? 'w-full justify-between bg-transparent rounded-md px-2 py-1 border border-white/10 hover:border-white/20 bg-black/50 hover:bg-black/60 px-4 py-3 text-white  text-xl'
            : 'bg-transparent rounded-md px-2 py-1 text-white  text-xl'
        }`}
        style={{
          ...
          <div className="py-2">
            {options.map((option) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left  text-base transition-colors duration-150 ${
                  option.value === value
